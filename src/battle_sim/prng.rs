...
impl LCRNG {
    ...
    pub(crate) fn generate_number_in_range(&mut self, range: RangeInclusive<u16>) -> u16 {
        ...
        let start = range_iter.next().expect("The range given to generate_number_in_range must have a first element.");
        let end = range_iter.next_back().expect("The range given to generate_number_in_range must have a last element.");
