...
use monsim::{
    effects, Count, EventFilteringOptions, EventHandler, Outcome, Percent, PersistentStatusDexEntry, PersistentStatusSpecies, PositionRelationFlags,
    VolatileStatusDexEntry, VolatileStatusSpecies, EVENT_HANDLER_REGISTRY,
};
...
pub const Burned: PersistentStatusSpecies = PersistentStatusSpecies::from_dex_entry(PersistentStatusDexEntry {
    ...
    bind_event_handlers: || {
        unsafe {
            EVENT_HANDLER_REGISTRY.register(
                |registry| &mut registry.on_calculate_attack_stat,
                EventHandler {
                    #[cfg(feature = "debug")]
        ...
        registry.register(
            |registry| &mut registry.on_turn_end,
            EventHandler {
                #[cfg(feature = "debug")]
                source_code_location: source_code_location!(),
...
pub const Confused: VolatileStatusSpecies = VolatileStatusSpecies::from_dex_entry(VolatileStatusDexEntry {
    ...
    bind_event_handlers: |registry| {
        registry.register(
            |registry| &mut registry.on_try_move,
            EventHandler {
                #[cfg(feature = "debug")]
                source_code_location: source_code_location!(),
