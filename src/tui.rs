...
use crate::{
    sim::{BattleSimulator, BattleState, FullySpecifiedChoice, MonsterID, PartiallySpecifiedChoice, PerTeam, TeamID, EMPTY_LINE},
    ActivationOrder, AvailableChoicesForTeam,
};
...
pub fn run(mut battle: BattleState) -> TuiResult<Nothing> {
    ...
    let mut current_app_state = AppState::AcceptingInput(InputMode::MidBattle(available_choices));
        ...
        match &mut current_app_state {
            AppState::AcceptingInput(input_mode) => {
                ...
                if let Some(pressed_key) = get_pressed_key(&receiver)? {
                    let optional_new_app_state = update_from_input(&mut ui, &mut battle, input_mode, &mut choices_for_turn, pressed_key);
            ...
            AppState::Simulating(choices) => {
                ...
                let maybe_fainted_active_battler = battle.active_monsters().into_iter().find(|monster| monster.is_fainted);
                if battle.is_finished {
                    ...
                    let activation_order = {
                        if let FullySpecifiedChoice::SwitchOut { activation_order, .. } = choices[fainted_battler.id.team_id] {
                            activation_order
                        } else {
                            unreachable!()
                        }
    ...
    match current_input_mode {
        InputMode::MidBattle(available_choices) => {
            match pressed_key {
                ...
                KeyCode::Up => {
                    ui.scroll_current_widget_up();
                    None
                }
                KeyCode::Down => {
                    ui.scroll_current_widget_down();
                    None
                }
                KeyCode::Left => {
                    ui.select_left_widget();
                    None
                }
                KeyCode::Right => {
                    ui.select_right_widget();
                    None
                }
                KeyCode::Enter => {
                    ...
                    if let Some((selected_menu_item_index, team_id)) = maybe_selected_menu_item {
                        ...
                        match selected_choice {
                            PartiallySpecifiedChoice::Move {
                                ...
                                choices_for_turn[team_id] = Some(FullySpecifiedChoice::Move {
                                    attacker_id,
                                    move_id,
                                    target_id,
                                    activation_order,
                            ...
                            PartiallySpecifiedChoice::SwitchOut {
                                active_monster_id,
        ...
        InputMode::SwitcheePrompt {
            is_between_turn_switch,
            active_monster_id,
            switchable_benched_monster_ids,
            highlight_cursor,
            activation_order,
            ...
            match pressed_key {
                KeyCode::Esc => Some(AppState::AcceptingInput(InputMode::MidBattle(battle.available_choices()))),
                ...
                KeyCode::Enter => {
                    ...
                    if *is_between_turn_switch {
                        ...
                        *choices_for_turn = PerTeam::both(None);
                        ...
                        choices_for_turn[active_monster_id.team_id] = Some(FullySpecifiedChoice::SwitchOut {
                            active_monster_id: *active_monster_id,
                            benched_monster_id,
        ...
        InputMode::PostBattle => match pressed_key {
            KeyCode::Esc => Some(AppState::Terminating),
