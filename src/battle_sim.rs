...
impl Battle {
    ...
    fn simulate_turn(&mut self, user_input: UserInput) -> BattleResult {
        ...
        for action_choice in  action_choices.into_iter() {
            ...
            result = match action_choice {
                ActionChoice::Move { move_uid, target_uid } => {
                    Action::damaging_move(&mut self.context, move_uid, target_uid)
                },
                ActionChoice::None => Err(BattleError::WrongState("No action was taken by a Monster.")),
            };
            // Check if any monster fainted due to the last action.
            if let Some(battler) = self.context.battlers().flatten().find(|it| {
                it.fainted()}) {
                let battler = *battler;
                Action::display_message(&mut self.context, &format!["{} fainted!", battler.monster.nickname]);  
    ...
    pub(crate) fn priority_sort<T:Clone+Copy>(prng: &mut LCRNG, vector: &mut Vec<T>, activation_order: &mut dyn FnMut(T) -> ActivationOrder) {
        
        // Sort without resolving speed ties, this sorting is stable, so it doesn't affect the order of condition-wise equal elements.
        vector.sort_by(|a, b| {
            activation_order(*a).cmp(&activation_order(*b)) 
        }
    );
    ...
    fn resolve_speed_tie<T:Clone+Copy>(prng: &mut LCRNG, vector: &mut Vec<T>, tied_monster_indices: &mut Vec<usize>) {
