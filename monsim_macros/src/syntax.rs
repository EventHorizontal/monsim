...
pub mod event_system_macro_syntax {
    ...
    use syn::{parenthesized, parse::{Parse, ParseStream}, token::Comma, Error, Expr, ExprStruct, Token};
    ...
    impl Parse for EventExpr {
        fn parse(input: ParseStream) -> syn::Result<Self> {
            ...
            if input.parse::<Token![;]>().is_ok() {
                ...
                assert!(settings.path.get_ident().expect("Expected settings struct to have an identifier").to_string() == "Settings", "Expected settings struct to have identifier `Settings`");
