...
impl<'a> AppState<'a> {
    fn new(battle: &mut Battle) -> Self {
        let mut state = Self {
            ...
            ally_active_battler_string: BattlerTeam::battler_status_as_string(battle.ally_team.active_battler()),
            ally_team_string: battle.ally_team.to_string(),
            opponent_active_battler_string: BattlerTeam::battler_status_as_string(battle.opponent_team.active_battler()),
                ...
                available_actions
                    ...
                    .map(|choice| {
                        ListItem::new(
                            battle_context
                                .move_({
                                    let ActionChoice::Move { move_uid, target_uid: _ } = choice;
                                    *move_uid
                                })
                                .species
                                .name,
...
impl App {
    pub fn run(mut battle_sim: BattleSimulator) -> MonsimResult {
        ...
        'app: loop {
            let should_exit = App::update_state_from_input(&mut terminal, &mut app_state, &mut battle_sim, &receiver)?;
            if should_exit {
                break 'app;
            }
            App::render_interface(&mut terminal, &mut app_state)?;
        }
    ...
    fn update_state_from_input(
        ...
        match app_state.app_mode {
            AppMode::AwaitingUserInput { ref available_actions } => {
                            ...
                            match (event.code, event.kind) {
                                ...
                                (KeyCode::Up, KeyEventKind::Release) => match app_state.selected_list {
                                    ScrollableWidgets::AllyTeamChoices => {
                                        if let Some(selected_index) = app_state.ally_list_state.selected() {
                                            let ally_list_length = app_state.ally_list_length();
                                            app_state
                                                .ally_list_state
                                                .select(Some((selected_index + ally_list_length - 1) % ally_list_length));
                                        } else {
                                            app_state.ally_list_state.select(Some(0));
                                        }
                                    }
                                    ScrollableWidgets::OpponentTeamChoices => {
                                        if let Some(selected_index) = app_state.opponent_list_state.selected() {
                                            let opponent_list_items_length = app_state.opponent_list_length();
                                            app_state
                                                .opponent_list_state
                                                .select(Some((selected_index + opponent_list_items_length - 1) % opponent_list_items_length));
                                        } else {
                                ...
                                (KeyCode::Down, KeyEventKind::Release) => match app_state.selected_list {
                                    ScrollableWidgets::AllyTeamChoices => {
                                        if let Some(selected_index) = app_state.ally_list_state.selected() {
                                            let ally_list_length = app_state.ally_list_length();
                                            app_state.ally_list_state.select(Some((selected_index + 1) % ally_list_length))
                                        } else {
                                            app_state.ally_list_state.select(Some(0));
                                        }
                                    }
                                    ScrollableWidgets::OpponentTeamChoices => {
                                        if let Some(selected_index) = app_state.opponent_list_state.selected() {
                                            let opponent_list_items_length = app_state.opponent_list_length();
                                            app_state.opponent_list_state.select(Some((selected_index + 1) % opponent_list_items_length))
                                        } else {
                                            app_state.opponent_list_state.select(Some(0));
                                        }
                                    }
                                    ScrollableWidgets::MessageLog => {
                                        app_state.message_log_scroll_idx = (app_state.message_log_scroll_idx + 1).min(battle_sim.battle.message_buffer.len());
                                    }
                                    ScrollableWidgets::AllyTeamStatus => todo!(),
                                    ScrollableWidgets::OpponentTeamStatus => todo!(),
                                },
                                (KeyCode::Left, KeyEventKind::Release) => match app_state.selected_list {
                                    ScrollableWidgets::AllyTeamStatus => todo!(),
                                    ScrollableWidgets::OpponentTeamStatus => todo!(),
                                    ScrollableWidgets::MessageLog => {
                                ...
                                (KeyCode::Right, KeyEventKind::Release) => match app_state.selected_list {
                                    ScrollableWidgets::AllyTeamStatus => todo!(),
                                    ScrollableWidgets::OpponentTeamStatus => todo!(),
                                    ScrollableWidgets::MessageLog => {
                                        app_state.selected_list = ScrollableWidgets::OpponentTeamChoices;
                                ...
                                (KeyCode::Tab, KeyEventKind::Release) => {
                                    if let (Some(selected_ally_choice_index), Some(selected_opponent_choice_index)) =
                                        (app_state.ally_list_state.selected(), app_state.opponent_list_state.selected())
                                    {
            ...
            AppMode::Simulating { ref chosen_actions } => {
                ...
                match result {
                    Ok(_) => {
                        battle_sim.battle.push_message(&"(The turn was calculated successfully.)");
                    }
    ...
    pub fn render_interface<'a>(terminal: &'a mut Terminal<CrosstermBackend<Stdout>>, app_state: &mut AppState) -> std::io::Result<CompletedFrame<'a>> {
        let terminal_height = terminal.size()?.height as usize;

        let longest_message_length = app_state.message_buffer.iter().fold(0usize, |acc, x| if x.len() > acc { x.len() } else { acc });
        ...
        terminal.draw(|frame| {
            ...
            let ally_stats_widget = Paragraph::new(app_state.ally_active_battler_string.as_str())
                .block(Block::default().title(" Ally Active Monster ").borders(Borders::ALL))
            ...
            let ally_choice_menu_widget = List::new(app_state.ally_list_items.clone())
                .block(
                    Block::default()
                        .title(if app_state.selected_list == ScrollableWidgets::AllyTeamChoices {
                            Span::styled(" Ally Monster Choices ", Style::default().fg(Color::Yellow))
                        } else {
                ...
                .highlight_symbol(">>");
            frame.render_stateful_widget(ally_choice_menu_widget, ally_panel_chunks[1], &mut app_state.ally_list_state);

            // Ally Team Roster Widget
            ...
            app_state.message_log_scroll_idx = app_state
                .message_log_scroll_idx
                .min(app_state.message_buffer.len().saturating_sub(terminal_height - 4));
            let text = app_state
                .message_buffer
                .iter()
                .enumerate()
            ...
            let paragraph_widget = Paragraph::new(text)
                .block(
                    Block::default()
                        .title(if app_state.selected_list == ScrollableWidgets::MessageLog {
            ...
            let opponent_stats_widget = Paragraph::new(app_state.opponent_active_battler_string.as_str())
                .block(Block::default().title(" Opponent Active Monster ").borders(Borders::ALL))
                .alignment(Alignment::Left)
                .wrap(Wrap { trim: true });
            ...
            let opponent_choice_menu_widget = List::new(app_state.opponent_list_items.clone())
                .block(
                    Block::default()
                        .title(if app_state.selected_list == ScrollableWidgets::OpponentTeamChoices {
                            Span::styled(" Opponent Monster Choices ", Style::default().fg(Color::Yellow))
                        } else {
                ...
                .highlight_style(Style::default().add_modifier(Modifier::ITALIC))
                .highlight_symbol(">>");
            frame.render_stateful_widget(opponent_choice_menu_widget, opponent_panel_chunks[1], &mut app_state.opponent_list_state);

            // Opponent Team Roster Widget
            let opponent_team_status_widget = Paragraph::new(app_state.opponent_team_string.as_str())
                .block(
                    Block::default()
                        .title(if app_state.selected_list == ScrollableWidgets::OpponentTeamStatus {
                            Span::styled(" Opponent Team Status ", Style::default().fg(Color::Yellow))
                        } else {
